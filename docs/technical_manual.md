...
Appendix K stands apart from that argument. Every other appendix validates the engine against a physical or notational reference — a synthesized tone of known frequency, a MIDI score, a second configuration of the same pipeline, or the corpus's own transcription of what was played. None establishes that the quantity being measured so carefully corresponds to what a trained musician hears. Appendix K states what the existing evidence does settle about that question, and marks clearly what it does not.

Appendix L then closes the loop between the manual and the study it supports. Every other URMP appendix runs the *shipped* defaults; L runs the configuration §1 actually recommends — Subsequence DTW with Adaptive RMS disabled, on short excerpts — and scores it against the corpus's independent ground-truth transcription rather than against the score alone.
...
3. **Short excerpts.** Keep each take short (**≤ ~2 minutes**). This is primarily a data-hygiene and reproducibility choice — short trimmed excerpts are easier to align, verify and re-run, and they bound the cost of any single bad take. It is *not* required for alignment stability: §6 shows the current $\Sigma_2$ pipeline aligns full-length stems under Subsequence without drift. Appendix L measures this excerpt regime directly, on 61 URMP excerpts cut at rests under the recommended configuration.
...
* **Recommendation**: Precise paired comparisons should be conducted in DTW mode (MIDI upload), as Legacy mode fundamentally lacks the ordinal anchors required to handle missing data without drift. DTW mode's note indices make a **paired** delta possible — the per-note difference averaged over the notes *both* conditions detected — which removes the drift by construction. The application reports that paired delta as the primary effect in the DTW summary, alongside the independent-means delta (kept for transparency; the two agree only when both takes detect the same notes) and a coverage line plus an advisory that fires when the two yields diverge. The paired figure is the one to report; the independent-means difference above is the drift it avoids.
...
The final column is the operative one and is derived in Appendix J. Order statistics can only return values on the pitch encoder's output grid, so on pYIN data their confidence intervals collapse to zero width and they must be read as naming a lattice cell rather than as measurements. That cell is $10$ cents wide in practice: the per-note median inherits the frame lattice intact rather than halving it, as Appendix J §2 measures directly over the full corpus. The trimmed mean is the recommended single robust figure because it averages the surviving values rather than selecting one, and so remains resolvable while still discarding the heavy tails.
...
> [!NOTE]
> **The mean-and-SD summary was insufficient, and is now supplemented rather than replaced.** Both deviation distributions are demonstrably non-normal, with excess kurtosis of $+4.18$ on per-note pYIN deviations and $+10.48$ at frame level (REAPER, the continuous control, reaches $+1.51$). Median, IQR, skewness and kurtosis are now computed and displayed in both Legacy and DTW modes, in cents and in Hertz, alongside distribution and Bland–Altman plots.
>
...
Every other appendix in this manual validates the engine against a physical or notational reference. Appendices E and G compare it to synthesized tones of exactly known frequency. Appendices A and D compare it to a MIDI score. Appendices B, F and H–J compare it to a second configuration of itself. Appendix L compares it to the URMP corpus's own ground-truth transcription of what was performed — the one reference used anywhere in this manual that was supplied neither by the engine's designers nor by the score. Each is a necessary check, and none of them — the independent transcription included, since it records played frequency rather than heard intonation — establishes that a deviation the system prints as $-38$ cents is one a trained string player would hear as flat.
...
1. **Segmentation at rests.** Stems longer than the $120$ s excerpt cap are cut into excerpts at note gaps $\ge 0.20$ s (a $0.10$ s margin is kept around each cut), greedily choosing the latest rest that keeps each excerpt within the cap. This reproduces the study's short-trimmed-excerpt format. The 41 stems yield **61 excerpts** (mean $73.5$ s, range $2.2$–$120.1$ s). The segmentation is lossless: the 61 excerpts span $9{,}496$ score notes in total, exactly the corpus note count reported independently by the §7 exclusion study, so no note is dropped at a cut boundary.

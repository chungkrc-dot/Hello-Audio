...
def test_condition(base_audio, instrument, engine, cents, tempdir, duration=None):
    y, sr = librosa.load(base_audio, sr=None, duration=duration)
    ...
    # We must write the truncated base audio too if duration is not None
    base_path = os.path.join(tempdir, f"base_{instrument}_{engine}.wav")
    sf.write(base_path, y, sr)
    
    with open(base_path, 'rb') as af:
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        # First, run REAPER on full tracks (it's very fast)
        for inst_name, path in datasets.items():
            for cents in conditions:
                meas, err = test_condition(path, engine_inst_map[inst_name], "REAPER", cents, tempdir, duration=None)
                print(f"{inst_name} | REAPER | {cents:+.0f}c | Meas: {meas:+.2f}c | Err: {err:+.2f}c")
        
        # Next, run pYIN on just 20 seconds to get the exact snapping direction
        for inst_name, path in datasets.items():
            for cents in conditions:
                meas, err = test_condition(path, engine_inst_map[inst_name], "pYIN", cents, tempdir, duration=20.0)
                print(f"{inst_name} | pYIN | {cents:+.0f}c | Meas: {meas:+.2f}c | Err: {err:+.2f}c")
